...
使用tenant_access_token和docx API
"""
...
print(f"  响应状态码: {doc_response.status_code}")
print(f"  响应内容: {doc_response.text[:500]}")

try:
    doc_result = doc_response.json()
except:
    print(f"✗ 解析响应失败，原始响应: {doc_response.text}")
    sys.exit(1)
...
document_info = doc_result.get("data", {})
document_id = document_info.get("document_id")
print(f"✓ 文档创建成功!")
print(f"  文档ID: {document_id}")
print(f"  文档标题: {document_info.get('title')}")
...
if add_result.get("code") != 0:
    ...
    if child_doc_result.get("code") != 0:
        ...
        child_document_info = child_doc_result.get("data", {})
        child_document_id = child_document_info.get("document_id")
        print(f"✓ 子文档创建成功!")
        print(f"  文档ID: {child_document_id}")
        
        # 将子文档添加到【贪吃蛇】下
        child_add_payload = {
            "obj_type": "docx",
            "obj_token": child_document_id,
